...
def main():
    ...
    try:
        ...
        rpc = '''
            <rpc>
                <get-path-computation-client-statistics>
                        <all/>
                </get-path-computation-client-statistics>
            </rpc>
            '''
        ...
        for i in content:  # iterates among all of nodes and sends rpc-requests
            if i['Is_Message_Broker_Channel_Open']:
                logger.info('Sending RPC -> Node Name: %s. RPC Routing Key: %s.' % (i['Node_Name'],
                                                                                    i['RPC_Routing_Key']))
                properties = pika.BasicProperties(reply_to=RPC_QUEUE, correlation_id=i['IPv4_Router_Identifier'])
...
if __name__ == '__main__':
    ...
    publisher.start()
    publisher.setName(HOSTNAME+'_PCCRPCRequester')
