...
def handler(update_message):
    query = '''
    MATCH ()-[r:Link {IPv4_Interface_Address:'%s'}]->()
      SET r.rtt = %i''' % (update_message['source_address'], int(update_message['rtt']))
...
def on_message_callback(ch, method, properties, body):
    ...
    probe_test_result = json.loads(body.decode('utf-8'))
    source_address = IPNetwork(probe_test_result['source_address']+'/30')
    target_address = IPNetwork(probe_test_result['target_address']+'/30')
    try:
        if source_address.network == target_address.network:  # Evaluates that the source and target belong to the same network
            handler(probe_test_result)
...
if __name__ == '__main__':
    ...
    rmq_collector.start()
    rmq_collector.setName(HOSTNAME+'_RPMDataConsumer')
    ...
    rmq_publisher_server.setName(HOSTNAME+'_RPMQueryPublisherServerQueue')
    ...
    rmq_publisher_client.setName(HOSTNAME+'_RPMQueryPublisherClientQueue')
